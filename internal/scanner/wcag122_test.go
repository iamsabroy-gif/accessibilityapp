...
func TestScoring_122_CriticalPenalty(t *testing.T) {
	...
	score, _, _ := scoring.Calculate(violations, 0, 0, scoring.FormulaPenalty)
	const want = 80
	if score != want {
...
func TestScoring_122_CriticalGrade(t *testing.T) {
	...
	_, grade, _ := scoring.Calculate(violations, 5, 0, scoring.FormulaPenalty)
	if grade != "B" {
...
func TestScoring_122_MultipleVideosMissingCaptions(t *testing.T) {
	...
	score, grade, _ := scoring.Calculate(violations, 0, 0, scoring.FormulaPenalty)
	const wantScore = 40
	if score != wantScore {
...
func TestScoring_122_CompliancePct(t *testing.T) {
	...
	_, _, pct := scoring.Calculate(violations, 3, 0, scoring.FormulaCompliance)
	const want = 75.0
	if pct != want {
...
func TestScoring_122_PerfectScoreWhenAllPass(t *testing.T) {
	t.Parallel()
	score, grade, pct := scoring.Calculate(nil, 5, 0, scoring.FormulaCompliance)
	if score != 100 {
